...
func TestPromptCacheBreakpointsRemovedForCodexCompatibility(t *testing.T) {
	inputJSON := []byte(`{
		"model":"gpt-5.2",
	...
	if gjson.GetBytes(output, "prompt_cache_options").Exists() {
		t.Fatal("prompt_cache_options should be removed")
	}
	if gjson.GetBytes(output, "input.0.content.0.prompt_cache_breakpoint").Exists() {
		t.Fatal("nested prompt_cache_breakpoint should be removed")
	}
