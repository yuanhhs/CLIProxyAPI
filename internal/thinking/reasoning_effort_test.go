...
func TestExtractReasoningEffortSupportsOpenAIChatCompletions(t *testing.T) {
	got := ExtractReasoningEffort([]byte(`{"reasoning_effort":"xhigh"}`), "openai", "gpt-5.4")
	if got != "xhigh" {
		t.Fatalf("ExtractReasoningEffort() = %q, want %q", got, "xhigh")
	}
}
