...
func TestOpenAICompatExecutorResponsesStreamRejectsEOFBeforeDone(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "text/event-stream")
		_, _ = w.Write([]byte(`data: {"id":"chatcmpl_1","object":"chat.completion.chunk","choices":[{"index":0,"delta":{"content":"partial"},"finish_reason":null}]}` + "\n\n"))
	}))
	defer server.Close()
	...
	if gotErr == nil || !strings.Contains(gotErr.Error(), "closed before [DONE]") {
		t.Fatalf("stream error = %v, want premature EOF error", gotErr)
	}
}
