...
func TestUsageReporterSetTranslatedReasoningEffortPreservesOriginalOpenAIEffort(t *testing.T) {
	ctx := usage.WithReasoningEffort(context.Background(), "high")
	reporter := NewUsageReporter(ctx, "openai-compatibility", "gpt-5.4", nil)

	// OpenAI-compatible translation or payload rules may remove reasoning_effort
	// even though it was present on the client request.
	reporter.SetTranslatedReasoningEffort([]byte(`{"model":"gpt-5.4"}`), "openai")

	record := reporter.buildRecord(usage.Detail{TotalTokens: 3}, false)
	if record.ReasoningEffort != "high" {
		t.Fatalf("reasoning effort = %q, want %q", record.ReasoningEffort, "high")
	}
}

func TestUsageReporterSetTranslatedReasoningEffortPrefersUpstreamEffort(t *testing.T) {
