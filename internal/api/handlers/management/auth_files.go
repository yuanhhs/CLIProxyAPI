...
func (h *Handler) GetAuthFileModels(c *gin.Context) {
	...
	if h.authManager != nil {
		...
		for _, auth := range auths {
			...
			matchesName := auth.FileName == name || auth.ID == name
			if strings.EqualFold(strings.TrimSpace(auth.Provider), "opencode") {
				matchesName = openCodeAuthNameMatches(auth.FileName, auth.ID, auth.Attributes, name)
			}
			if matchesName {
				authID = auth.ID
				targetAuth = auth
	...
	if targetAuth != nil && strings.EqualFold(strings.TrimSpace(targetAuth.Provider), "opencode") {
		h.getOpenCodeAuthFileModels(c, targetAuth)
		return
	}
