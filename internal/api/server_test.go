...
func newTestServer(t *testing.T) *Server {
	...
	t.Cleanup(func() {
		internalusage.SetStore(nil)
		if server.usageStore != nil {
			_ = server.usageStore.Close()
		}
	})
	return server
}
...
func TestManagementUsageEndpointsRequireAuth(t *testing.T) {
	...
	var usagePayload struct {
		Usage struct {
			...
			APIs          map[string]internalusage.APISnapshot `json:"apis"`
		} `json:"usage"`
	}
	...
	if usagePayload.Usage.APIs == nil {
		t.Fatalf("usage response is missing official snapshot fields: %s", usageRR.Body.String())
	}
