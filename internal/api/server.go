...
func NewServer(cfg *config.Config, authManager *auth.Manager, accessManager *sdkaccess.Manager, configFilePath string, opts ...ServerOption) *Server {
	...
	// Debug-only unit servers commonly use temporary directories and do not enable
	// usage statistics; avoid opening a SQLite handle for those short-lived engines.
	if cfg == nil || !cfg.Debug || cfg.UsageStatisticsEnabled {
		usageCtx, usageCancel := context.WithTimeout(context.Background(), 10*time.Second)
		if store, err := usage.Open(usageCtx, configFilePath); err != nil {
			log.WithError(err).Warn("usage analytics database unavailable")
		} else {
			usageStore = store
...
func (s *Server) registerManagementRoutes() {
	...
	mgmt.Use(s.managementAvailabilityMiddleware(), s.mgmt.Middleware())
	{
		...
		mgmt.POST("/opencode/quota", s.mgmt.GetOpenCodeQuota)
...
func (s *Server) Stop(ctx context.Context) error {
	...
	if s.usageStore != nil {
		...
		if err := s.usageStore.Close(); err != nil {
			log.WithError(err).Warn("failed to close usage database")
		}
	}
