...
func TestFileSynthesizer_Synthesize_OpenCodeRuntimeAttributes(t *testing.T) {
	tempDir := t.TempDir()
	authData := map[string]any{
		"type":    "opencode",
	...
	data, _ := json.Marshal(authData)
	if errWrite := os.WriteFile(filepath.Join(tempDir, "opencode-auth.json"), data, 0644); errWrite != nil {
	...
	if got := auth.Attributes["api_key"]; got != "test-key" {
		t.Fatalf("api_key = %q, want trimmed key", got)
	}
	if got := auth.Attributes["base_url"]; got != openCodeAPIBaseURL {
		t.Fatalf("base_url = %q, want %q", got, openCodeAPIBaseURL)
	}
