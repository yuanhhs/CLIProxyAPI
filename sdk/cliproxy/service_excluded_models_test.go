...
func TestRegisterModelsForAuth_OpenCode(t *testing.T) {
	...
	service.registerModelsForAuth(auth)
	models := modelRegistry.GetModelsForClient(auth.ID)
	if len(models) != len(internalregistry.GetOpenCodeModels()) {
		t.Fatalf("registered model count = %d, want %d", len(models), len(internalregistry.GetOpenCodeModels()))
	}
	for _, model := range models {
	...
	t.Fatal("expected kimi-k3 in registered OpenCode models")
}

func TestRegisterModelsForAuth_OpenCodeWithoutAPIKey(t *testing.T) {
	service := &Service{cfg: &config.Config{}}
	auth := &coreauth.Auth{
		ID:         "auth-opencode-quota-only",
		Provider:   "opencode",
		Status:     coreauth.StatusActive,
	...
	if models := modelRegistry.GetModelsForClient(auth.ID); len(models) != 0 {
		t.Fatalf("registered %d models for quota-only OpenCode auth, want 0", len(models))
	}
}
