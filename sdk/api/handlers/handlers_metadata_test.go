...
func TestSetReasoningEffortMetadataSupportsOpenAIChatCompletions(t *testing.T) {
	meta := make(map[string]any)

	setReasoningEffortMetadata(meta, "openai", "gpt-5.4", []byte(`{"reasoning_effort":"xhigh"}`))

	if got := meta[coreexecutor.ReasoningEffortMetadataKey]; got != "xhigh" {
		t.Fatalf("ReasoningEffortMetadataKey = %v, want %q", got, "xhigh")
	}
}
